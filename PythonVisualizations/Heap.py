...
class Heap(VisualizationApp):
    ...
    def __init__(self, size=2, maxArgWidth=MAX_ARG_WIDTH, title="Heap", **kwargs):
        ...
        self.setButtonsStatus(DISABLED, "Heapify")
    ...
    def removeMax(self):
        ...
        self.moveItemsBy(items, delta=(0, -max(400, self.canvas.coords(n.display_shape)[3])), steps=self.CELL_HEIGHT, sleepTime=.05)
    ...
    def trickleDown(self, cur = 0, size = -1):
        ...
        self.list[cur] = top  # move original root to its correct position
        self.cleanUp(callEnviron)
    ...
    def randomFill(self, val):
        ...
        for i in range(self.size):
            self.insert(random.randint(0, 99))
    ...
    def clickRemoveMax(self):
        if len(self.list) == 0:
            ...
            self.setButtonsStatus(NORMAL, "Heapify")
