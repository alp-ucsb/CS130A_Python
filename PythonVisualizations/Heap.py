...
class Heap(VisualizationApp):
    ...
    def insert(self, val):
        ...
        if len(self.list) <= self.nItems:
            ...
            for i in range(len(self.list)):
                ...
                self.createArrayCell(i + len(self.list)) # Lasting
            self.list+=[None]*(len(self.list)*2) #grow the array in implicit memory (double its size capacity)
        ...
        cellCoords = self.cellCoords(self.nItems) # Color box
        cellCenter = self.cellCenter(self.nItems) # Number in box
        ...
        self.nItems += 1       #increment count of how many cells are in use
        callEnviron -= set(cellPair)
    ...
    def siftUp(self, i):
        ...
        while 0 < i:
            ...
            if (self.list[parent] < item): # if parent less than item sifting up
                ...
                print("this is parent!", self.list[parent].display_shape)
    ...
    def newArray(self):
        ...
        del self.list[:]            #maybe dont need this
        self.list = [None]*2
        self.display()
    ...
    def removeMax(self):
        ...
        padding = 20
        canvasDimensions = self.widgetDimensions(self.canvas)
        cellCoords = (self.HEAP_X0 + self.CELL_WIDTH + padding, canvasDimensions[1] - self.CELL_HEIGHT - padding,
                      self.HEAP_X0 + self.CELL_WIDTH*2 + padding, canvasDimensions[1] - self.CELL_HEIGHT*2 - padding)
        cellCenter = (self.HEAP_X0 + self.CELL_WIDTH + padding + self.CELL_WIDTH/2,
                      canvasDimensions[1] - self.CELL_HEIGHT - padding- self.CELL_HEIGHT/2)
        toPositions = cellCoords, cellCenter

        self.moveItemsTo(items, toPositions, steps=self.CELL_HEIGHT, sleepTime=0.01)
    ...
    def swapRoot(self):
        ...
        toPositions = (cellCoords, cellCenter)
        startAngle = 90 * 50 / (50 + abs(cellCoords[0])) * (-1 if cellCoords[0] < 0 else 1)
        self.moveItemsOnCurve(items, toPositions, startAngle=startAngle)
        self.list[0] = n
        # set the last element of the list to None
        self.list[-1] = None
    ...
    def randomFill(self, val):
        ...
        del self.list[:]        #Also might not need this
        self.list = [None]*val
        self.display()
    ...
    def display(self):
        ...
        for i in range(len(self.list)):  # Draw grid of cells
            self.createArrayCell(i)

        #make a new arrow pointing to the top of the Heap
        self.nItemsIndex = self.createIndex(self.nItems)
    ...
    def fixPositions(self):  # Move canvas display items to exact cell coords
        ...
        for i, item in enumerate(self.canvas.find_withtag('arrayCell')):
            if i >= len(self.list): # Delete any extra cell borders
...
#REFERENCE COPY OF THE HEAP CODE BEFORE THE SWITCH WAS MADE TO self.nItems INSTEAD OF self.size:
'''import time
from tkinter import *
import random
...
class Heap(VisualizationApp):
    ...
    def peek(self):
        ...
        self.highlightCodeTags([], callEnviron)
        # finish animation
        print("call environment5th", callEnviron)
        self.cleanUp(callEnviron)
